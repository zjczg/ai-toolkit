...
def generate(
    ...
    **kwargs: Any,
    ...
    Supported model aliases: ``seedream-5-lite`` and ``seedream-4.5``.
    Local reference images are uploaded to a temporary public URL before the
    ARK request.
    """
    settings = get_settings()
    resolved_model = resolve_model(model)
    request_kwargs = _drop_none_values(kwargs)
    request_kwargs.setdefault("size", size or settings.ark_image_size)
    ...
    refs = references or []
    if resolved_model == SEEDREAM_5_MODEL:
        ...
        _enforce_reference_limit(refs, max_refs=14, model=resolved_model)
